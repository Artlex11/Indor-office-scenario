...
ASD_NLOS = open("D:\\Всё моё\\Радиофак\\Практика\\РАДИО МОДУЛЬ\\ЗАДАНИЕ 1\\C++\\Graphics\\LSP_NLOS\\PDF\\ASD_NLOS_PDF.txt", 'r')

ASD_NLOS = ASD_NLOS.read()
numbers = list(map(float, ASD_NLOS.split()))
...
plt.subplot(1, 2, 2)
plt.scatter(grad, p)
plt.title("PDF for ASD_NLOS (deg)")
