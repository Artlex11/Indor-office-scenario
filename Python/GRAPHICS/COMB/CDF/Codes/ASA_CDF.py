...
numbers_LOS = list(map(float, ASA_LOS.split()))
...
# график для NLOS
ASA_NLOS = open("D:\\Всё моё\\Радиофак\\Практика\\РАДИО МОДУЛЬ\\ЗАДАНИЕ 1\\C++\\Graphics\\LSP_NLOS\\CDF\\ASA_NLOS_CDF.txt", 'r')

ASA_NLOS = ASA_NLOS.read()
numbers_NLOS = list(map(float, ASA_NLOS.split()))
...
plt.title("CDF for ASA_LOS and ASA_NLOS")
plt.xlabel("ASA (rad)")
plt.ylabel("p")
plt.grid()
