...
# график для LOS
ZSD_LOS = open("D:\\Всё моё\\Радиофак\\Практика\\РАДИО МОДУЛЬ\\ЗАДАНИЕ 1\\C++\\Graphics\\LSP_LOS\\CDF\\ZSD_LOS_CDF.txt", 'r')

ZSD_LOS = ZSD_LOS.read()
numbers_LOS = list(map(float, ZSD_LOS.split()))
...
numbers_NLOS = list(map(float, ZSD_NLOS.split()))
...
plt.title("CDF for ZSD_LOS and ZSD_NLOS")
plt.xlabel("ZSD (rad)")
plt.ylabel("p")
plt.grid()
