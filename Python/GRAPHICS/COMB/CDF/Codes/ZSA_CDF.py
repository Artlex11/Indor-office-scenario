...
# график для LOS
ZSA_LOS = open("D:\\Всё моё\\Радиофак\\Практика\\РАДИО МОДУЛЬ\\ЗАДАНИЕ 1\\C++\\Graphics\\LSP_LOS\\CDF\\ZSA_LOS_CDF.txt", 'r')

ZSA_LOS = ZSA_LOS.read()
numbers_LOS = list(map(float, ZSA_LOS.split()))
...
# график для NLOS
ZSA_NLOS = open("D:\\Всё моё\\Радиофак\\Практика\\РАДИО МОДУЛЬ\\ЗАДАНИЕ 1\\C++\\Graphics\\LSP_NLOS\\CDF\\ZSA_NLOS_CDF.txt", 'r')

ZSA_NLOS = ZSA_NLOS.read()
numbers_NLOS = list(map(float, ZSA_NLOS.split()))
...
plt.title("CDF for ZSA_LOS and ZSA_NLOS")
plt.xlabel("ZSA (rad)")
plt.ylabel("p")
plt.grid()
