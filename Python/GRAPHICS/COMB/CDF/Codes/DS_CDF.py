...
DS_LOS = DS_LOS.read()
numbers_LOS = list(map(float, DS_LOS.split()))
...
# график для NLOS
DS_NLOS = open("D:\\Всё моё\\Радиофак\\Практика\\РАДИО МОДУЛЬ\\ЗАДАНИЕ 1\\C++\\Graphics\\LSP_NLOS\\CDF\\DS_NLOS_CDF.txt", 'r')

DS_NLOS = DS_NLOS.read()
numbers_NLOS = list(map(float, DS_NLOS.split()))
...
plt.title("CDF for DS_LOS and DS_NLOS")
plt.xlabel("DS")
plt.ylabel("p")
plt.grid()
plt.show()
